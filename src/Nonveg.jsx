...
function Nonveg() {
  ...
  const totalPages = Math.ceil(filteredNonVeg.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentNonVegItems = filteredNonVeg.slice(startIndex, startIndex + itemsPerPage);
  ...
  return (
    <div className="container my-4">
      ...
      <div className="row">
        {currentNonVegItems.length > 0 ? (
          currentNonVegItems.map((item, index) => (
          ...
          <p className="text-center">No matching non-veg items found.</p>
        )}
      </div>
